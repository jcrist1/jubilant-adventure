...
async fn main() -> Result<()> {
    ...
    env::set_current_dir("../adventurous_frontend")?;

    // TODO: config
    let pool = sqlx::PgPool::connect("postgresql://blog_user:secret123@localhost:5436/blog")
        .await
        .unwrap();
    ...
    let blog_service = blog_backend::BlogService::new(Arc::new(pool));
    let app = Router::new()
        .nest(
            "/api/v1",
            Router::new()
